...
- **OAuth start returns `429`:** reduce retries and check for abusive or misconfigured clients. `Retry-After: 600` indicates client-tracking capacity saturation; `Retry-After: 1` indicates pending OAuth-state capacity saturation. Limits and capacities reset on API restart. If a CDN or load balancer was added, review the trusted-proxy design rather than accepting its forwarded-address chain by default.
